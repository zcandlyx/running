<template>
	<view class="wrap" v-cloak :data-theme="appTheme">
		<cu-custom bgColor="bg-gradual-green"><block slot="content" class="text-white">首页</block></cu-custom>
		<view class="">
			<!-- <navigator url="../test/index"><button type="warn">前去体验变身</button></navigator> -->
			<navigator url="../login/index">去login</navigator>
		</view>
		<view class="" style="height:90rpx;" :data-theme="appTheme" @tap="toggleAppTheme('blue')">测试全局主题色</view>
		<view class="module-box" :style="'background:url(' + bg + ')no-repeat left top /100% 100%'">
			<view class="module">
				<view class="title">
					我的运动步数
					<button open-type="share" v-if="isGetStep">好友PK</button>
				</view>
				<view class="avatar-box"><open-data type="userAvatarUrl"></open-data></view>
				<view class="show-box">
					...
					<view class="show-box-sub">
						<text>{{ ranking != -1 ? ranking : '获取失败' }}</text>
						<text>当天排名</text>
					</view>
				</view>
		...
		<neil-modal :show="show" @close="closeModal" align="center" title="提示" content="您还未登录,请先登录" @cancel="bindBtn(0)" @confirm="bindBtn(1)"></neil-modal>
	</view>
</template>
...
export default {
	...
	computed: {
		...mapGetters({ bg: 'home_bg', appTheme: 'appTheme' }),
